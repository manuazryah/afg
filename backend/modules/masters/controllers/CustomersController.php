...
class CustomersController extends Controller {
    ...
    public function actionInventoryContent() {
        ...
        if (Yii::$app->request->isAjax) {
            ...
            $customers = \common\models\Customers::findOne($_POST['customer']);
            $customer_vehicles = \common\models\VehicleTowingInfo::find()->where(['customers_id' => $_POST['customer']])->all();
            $vehicle_id = array();
            foreach ($customer_vehicles as $vehicle) {
                $vehicle_details = \common\models\Vehicle::findOne($vehicle->vehicle_id);
                $vehicle_id[] = $vehicle->vehicle_id;
    ...
    public function actionInventoryReport($id) {
        $customers = \common\models\Customers::findOne($id);
        $customer_vehicles = \common\models\VehicleTowingInfo::find()->where(['customers_id' => $id])->all();
        $vehicle_id = array();
        foreach ($customer_vehicles as $vehicle) {
            $vehicle_details = \common\models\Vehicle::findOne($vehicle->vehicle_id);
        ...
        $content = $this->renderPartial('inventory_report_pdf', [
            'customer' => $customers,
            'vehicle_id' => $vehicle_id,
        ]);
        $pdf = new Pdf([
