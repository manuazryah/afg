...
<div class="vehicle-form form-inline">
    ...
    <div class="row">
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <?php // $form->field($vehicle_towing, 'condition')->dropDownList(['Non-OP' => 'Non-OP', 'Operable' => 'Operable',], ['prompt' => '']) ?>
            <?= $form->field($vehicle_towing, 'condition')->radioList(['Non-OP' => 'Non-OP', 'Operable' => 'Operable',]) ?>
        </div>
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <?php // $form->field($vehicle_towing, 'damaged')->dropDownList(['Yes' => 'Yes', 'No' => 'No',], ['prompt' => '']) ?>
            <?= $form->field($vehicle_towing, 'damaged')->radioList(['Yes' => 'Yes', 'No' => 'No',]) ?>
        </div>
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <label class="control-label">PICTURES</label>
            <?php // $form->field($vehicle_towing, 'pictures')->dropDownList(['Yes' => 'Yes', 'No' => 'No',], ['prompt' => '']) ?>
            <?php
            $check = FALSE;
            if (isset($vehicle_towing->pictures)) {
                if ($vehicle_towing->pictures == 'Yes')
                    $check = TRUE;
            }
            ?>
            <?= $form->field($vehicle_towing, 'pictures')->checkbox(['label' => null, 'checked' => $check]) ?>
        </div>
    </div>

    <div class="row">
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <label class="control-label">TOWED</label>
            <?php // $form->field($vehicle_towing, 'towed')->dropDownList(['Yes' => 'Yes', 'No' => 'No',], ['prompt' => '']) ?>
            <?php
            $towed = FALSE;
            ...
            ?>
            <?= $form->field($vehicle_towing, 'towed')->checkbox(['label' => null, 'checked' => $towed]) ?>
        </div>
        <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
            <?php // $form->field($vehicle_towing, 'keys')->dropDownList(['Yes' => 'Yes', 'No' => 'No',], ['prompt' => '']) ?>
            <label class="control-label">KEYS</label>
            <?php
            $keys = FALSE;
            if (isset($vehicle_towing->keys)) {
                if ($vehicle_towing->keys == 'Yes')
                    $keys = TRUE;
            }
            ?>
            <?= $form->field($vehicle_towing, 'keys')->checkbox(['label' => null, 'checked' => $keys]) ?>
        </div>
    </div>
    ...
    <div class="row">
        ...
        <?php if (Yii::$app->user->identity->post_id == 1) { ?>
            <div class='col-md-4 col-sm-6 col-xs-12 left_padd'>
                <?= $form->field($model, 'location')->dropDownList(yii\helpers\ArrayHelper::map(common\models\Location::find()->where(['status' => 1])->all(), 'id', 'location'), ['prompt' => '--Select--']) ?>
            </div>
        <?php } ?>
